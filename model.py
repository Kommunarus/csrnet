...
class CSRNet(nn.Module):
    def __init__(self, load_weights=False):
        ...
        if not load_weights:
            ...
            # Получаем state_dict — это обычные словари
            pretrained_dict = mod.state_dict()
            frontend_dict = self.frontend.state_dict()

            # Оставляем только слои, совпадающие по именам и размерам
            # (вдруг версии VGG или настройки отличаются)
            matched_dict = {k: v for k, v in pretrained_dict.items() if
                            k in frontend_dict and v.size() == frontend_dict[k].size()}

            # Обновляем только нужные веса
            frontend_dict.update(matched_dict)
